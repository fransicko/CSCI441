...
GLuint compileShader( const char* filename, GLenum shaderType ) {
    ...
    // TODO #1
	if ((shaderHandle = glCreateShader(shaderType)) <= 0) {
		fprintf(stderr, "There shader is not greater then zero.\n");
		return -1;
	}
...
GLuint createShaderProgram( const char *vertexShaderFilename, const char *fragmentShaderFilename ) {
    ...
    // TODO #4
	if ((shaderProgramHandle = glCreateProgram()) <= 0) {
		fprintf(stderr, "The shader program handle is less then 0\n");
		return -1;
	}
